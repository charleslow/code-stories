...
a markdown explanation. The story should flow like a guided tour from a friendly,
knowledgeable colleague - not a dry reference manual.
...
- For each key code region, note whether it is clean (pure logic) or noisy
  (interspersed with debug prints, commented-out code, verbose logging)
- For noisy regions, identify the cleanest contiguous sub-ranges that capture
  the essential logic
...
- If the query or codebase uses specialized terminology (e.g., "JEPA", "transformer",
  "einops"), the overview chapter MUST briefly define these terms for readers who may
  not already know them. Don't assume domain knowledge.
...
- If the relevant code spans multiple files, ensure the story visits at least 2-3
  different files to give the reader a sense of the codebase structure. If the relevant
  code is concentrated in a single file, acknowledge this early and organize chapters
  around logical sections within that file.
- If the story naturally divides into distinct phases or subsystems (e.g., a basic
  mechanism then an advanced variant, or a data pipeline then a training loop),
  introduce the transition between phases explicitly. Include a brief "bridge"
  sentence or paragraph explaining WHY the story is moving to the next phase and
  what problem or question the new phase addresses. Do not let the reader feel
  dropped into a new topic without context.
- End the story naturally. If the last code chapter provides a satisfying conclusion,
  you may omit a separate summary chapter entirely. If you do include a closing chapter,
  keep it SHORT (2-4 sentences, ~100 words max) — do NOT restate points already made in
  earlier chapters. Avoid the "essay conclusion" pattern.
...
7. **Jargon**: Is every technical term introduced or defined before first use?
8. **File coverage**: Does the story visit multiple files (if the codebase warrants it)?
9. **Closing**: Is the final chapter genuinely necessary? If it would just restate earlier
   points, remove it and let the last code chapter serve as the ending.
10. **Snippet quality (preview)**: Based on each chapter's "What to show" description,
    will any chapters need to show code regions that contain significant debug/logging
    code (more than ~10% of lines)? If so, note alternative line ranges or suggest
    splitting the chapter to isolate cleaner code segments.
11. **Phase transitions**: If the story has distinct phases or subsystems, is there a
    clear bridge between them explaining why the story is shifting focus?
...
- Each chapter's total code should be 20-70 lines (fits 1-2 screens). Never exceed
  ...
  code than 70 lines, split it into two chapters.
...
- Snippets MUST be free of noise: debug/logging statements, commented-out code,
  ...
  captures the essential logic (even if it's only 10-15 lines), and describe the
...
3. **Highlights key details**: Point out important lines, patterns, or decisions
   Reference specific lines in the code snippets (e.g., "On line 45..." or
   "Lines 12-15 show...") when it helps the reader connect explanation to code.
   Do this consistently across ALL chapters that have snippets.
...
- The overview chapter (first) has empty snippets array, just explanation. Keep the
  overview to 150-200 words. Define key terms concisely (one sentence each, not full
  paragraphs). The overview should orient the reader, not teach — the teaching happens
  in subsequent chapters.
- Use varied transitions between chapters. "Building on...", "Notice how...", and
  "This is where..." are fine starting points, but don't use the same transition
  pattern more than twice in a story. Some chapters can transition implicitly
  through content flow rather than explicit bridge phrases.
